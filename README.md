...
- **[skrashevich/meshtastic-firmware](https://github.com/skrashevich/meshtastic-firmware)** — Форк официальной прошивки Meshtastic с поддержкой русского языка. Включает: кириллический OLED-дисплей (флаг `OLED_RU`), полную русскую раскладку клавиатуры для T-Deck с переключением EN/RU (Shift+Mic), загрузку онлайн-тайлов карт при наличии Wi-Fi.  
  *Fork of the official Meshtastic firmware with Russian language support: Cyrillic OLED display, Russian keyboard for T-Deck, online map tile fallback over Wi-Fi.*  
  `C++` | Лицензия: GPL-3.0
...
- **[skrashevich/device-ui](https://github.com/skrashevich/device-ui)** — Форк библиотеки [meshtastic/device-ui](https://github.com/meshtastic/device-ui) для TFT и OLED экранов. Добавляет: загрузку онлайн-тайлов карт по WLAN/HTTPS (OpenStreetMap), русскую виртуальную клавиатуру для T-Deck с аппаратной поддержкой кирилличных символов, переключение RU/EN раскладки.  
  *Fork of meshtastic/device-ui with online map tile loading, Russian virtual/hardware keyboard for T-Deck, and Cyrillic layout toggle.*  
  `C++` | Лицензия: GPL-3.0

## Инструменты / Tools

- **[playmean/mtsh](https://github.com/playmean/mtsh)** — Удалённая оболочка (remote shell) через Meshtastic. Позволяет выполнять shell-команды на удалённом узле через текстовые сообщения mesh-сети. Поддерживает serial, BLE и HTTP подключение к устройству; chunked-передача больших ответов, адаптивное сжатие, белые списки узлов.  
  *Remote shell over Meshtastic text messages. Run shell commands on a remote node via the mesh. Supports serial/BLE/HTTP, chunked output, adaptive compression, node whitelisting.*  
  `Go` | Лицензия: MIT
...
- **[skrashevich/go-meshtastic-serial2tcp](https://github.com/skrashevich/go-meshtastic-serial2tcp)** — TCP ↔ Serial мост для устройств Meshtastic. Слушает на TCP-порту и пересылает фреймы Meshtastic на серийный порт. Поддерживает несколько одновременных клиентов, режим read-only для вторичных клиентов, mDNS-обнаружение, Docker-образ.  
  *Small TCP ↔ serial bridge for Meshtastic devices. Multiple TCP clients supported, read-only mode, mDNS discovery, Docker image available.*  
  `Go` | Лицензия: GPL-3.0

## Инфраструктура / Infrastructure

- **[capricornusx/meshtastic-mqtt-exporter](https://github.com/capricornusx/meshtastic-mqtt-exporter)** — Экспорт телеметрии Meshtastic-устройств в Prometheus. Встроенный MQTT-брокер (mochi-mqtt), поддержка TLS/QoS, метрики батареи/температуры/влажности/давления/RSSI/SNR, интеграция с AlertManager для отправки алертов в LoRa mesh-сеть, персистентность метрик.  
  *Export Meshtastic device telemetry to Prometheus. Built-in mochi-mqtt broker, TLS/QoS support, battery/temp/humidity/pressure/RSSI/SNR metrics, AlertManager integration to push alerts into the LoRa mesh.*  
  `Go` | Лицензия: MIT

## Сборщики / Build Tools

- **[skrashevich/meshtastic-firmware-builder](https://github.com/skrashevich/meshtastic-firmware-builder)** — Веб-приложение для сборки прошивки Meshtastic из неофициальных форк-репозиториев. Позволяет указать URL репозитория, выбрать ветку/тег, выбрать устройство и получить готовый бинарный файл прошивки. Бэкенд на Go, фронтенд на React/TypeScript, сборщик на PlatformIO в Docker. Поддержка RU/EN интерфейса, live-логи сборки.  
  *Web application for building Meshtastic firmware from non-official fork repositories. Provide a repo URL, pick a branch/tag and device, get firmware binaries. Go backend, React/TS frontend, PlatformIO builder in Docker, live build logs, RU/EN UI.*  
  `Go` / `TypeScript`

## Сообщество / Community

- **[Telegram: meshtastic_russia](https://t.me/meshtastic_russia)** — Основной русскоязычный Telegram-чат сообщества Meshtastic.  
  *Main Russian-speaking Meshtastic community Telegram chat.*
